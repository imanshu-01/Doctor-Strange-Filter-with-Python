...
def load_images(config: dict) -> tuple:
    ...
    inner_path = os.path.join(
        base_dir, config["overlay"]["inner_circle_path"]
    )
    outer_path = os.path.join(
        base_dir, config["overlay"]["outer_circle_path"]
    )

    inner_circle = cv.imread(inner_path, cv.IMREAD_UNCHANGED)
    outer_circle = cv.imread(outer_path, cv.IMREAD_UNCHANGED)

    if inner_circle is None or outer_circle is None:
        raise FileNotFoundError(
            f"Failed to load overlay images:\n{inner_path}\n{outer_path}"
        )
